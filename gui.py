...
class MetaXtract_GUI(QMainWindow):
    def __init__(self):
        ...
        ms2_buttons = QHBoxLayout()
        ms2_select_all = QPushButton("Select All (MS2)", self)
        ms2_clear_selection = QPushButton("Clear Selection (MS2)", self)
        ms2_select_all.clicked.connect(lambda: [cb.setChecked(True) for cb in self.scan_header_options])
        ms2_clear_selection.clicked.connect(lambda: [cb.setChecked(False) for cb in self.scan_header_options])
        ms2_buttons.addWidget(ms2_select_all)
        ms2_buttons.addWidget(ms2_clear_selection)
        ms2_l.addLayout(ms2_buttons)
        ...
        ms1_buttons = QHBoxLayout()
        ms1_select_all = QPushButton("Select All (MS1)", self)
        ms1_clear_selection = QPushButton("Clear Selection (MS1)", self)
        ms1_select_all.clicked.connect(lambda: [cb.setChecked(True) for cb in self.scan_header_options_ms1])
        ms1_clear_selection.clicked.connect(lambda: [cb.setChecked(False) for cb in self.scan_header_options_ms1])
        ms1_buttons.addWidget(ms1_select_all)
        ms1_buttons.addWidget(ms1_clear_selection)
        ms1_l.addLayout(ms1_buttons)
